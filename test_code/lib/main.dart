              ...
              children: [
                ConstrainedBox(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(fontSize: 17, fontStyle: FontStyle.italic, color: Colors.grey[200]),
                  ),
                ),
                const SizedBox(height: 20),
                GestureDetector(
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      // After registration → ProfilePage
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    if (_formKey.currentState!.validate()) {
      ...
      // After login → ProfilePage (NOT HomeScreen)
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (context) => const ProfilePage()),  // CHANGED THIS LINE
        (route) => false,
      );
    }
  }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (context) => const ProfilePage()),  // After test → ProfilePage
                (route) => false,
              );
            },
            style: ElevatedButton.styleFrom(
